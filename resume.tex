...
\textbf{Fiş OCR ile Masraf Takip Uygulaması (Web ve Mobil)} \hfill \textit{Python, FastAPI, PostgreSQL, OpenAI, Flutter}
\begin{itemize}[leftmargin=*,noitemsep,topsep=0pt]
    \item Fiş fotoğrafından tutar, KDV ve kalemleri otomatik okuyan yapay zeka destekli OCR servisi geliştirildi
    \item Birden fazla şirketin ayrı ayrı kullanabildiği web ve mobil masraf takip uygulaması geliştirildi
    \item SAP'den gelen fişlerin toplu olarak işlenmesi için entegrasyon yapıldı
\end{itemize}
\textbf{Yapay Zeka Destekli Sosyal Medya İçerik Otomasyonu} \hfill \textit{Next.js, PostgreSQL}
\begin{itemize}[leftmargin=*,noitemsep,topsep=0pt]
    \item Marka kimliğine uygun görsel ve metin üreten, onay sonrası LinkedIn/Instagram/Facebook'ta otomatik paylaşan sistem tasarlandı
    \item Paylaşım performansını ölçüp sonraki içerikleri iyileştiren döngü ve özel günlere göre otomatik içerik planı tasarlandı
\end{itemize}
\textbf{Ticket Odaklı Yapay Zeka Yazılım Üretim Hattı} \hfill \textit{TypeScript, Claude Agent SDK, SQLite, Git}
\begin{itemize}[leftmargin=*,noitemsep,topsep=0pt]
    \item Bir iş talebini yapay zeka ajanlarıyla otomatik olarak koda dönüştürüp incelemeye hazır hale getiren yerel sistem geliştirildi
    \item Plan ve kod inceleme aşamalarında insan onayı alan, yarım kalan işleri güvenle sürdürebilen yapı tasarlandı
\end{itemize}
